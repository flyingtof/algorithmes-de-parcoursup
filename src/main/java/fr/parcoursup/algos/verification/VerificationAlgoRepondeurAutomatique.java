...
public class VerificationAlgoRepondeurAutomatique {
    ...
    public static void verifierP71etP74(Collection<Voeu> voeux, Set<Integer> candidatsAvecRepondeurAutomatique) throws VerificationException {
        ...
        for (Voeu v : voeux) {
            ...
            if (candidatsAvecRepondeurAutomatique.contains(v.id.gCnCod)
                    && !v.aEteProposeJoursPrecedents()
                    && v.getStatut() != Voeu.StatutVoeu.REP_AUTO_REFUS_PROPOSITION
    ...
    public static void verifierP72(Collection<Voeu> voeux, Set<Integer> candidatsAvecRepondeurAutomatique) throws VerificationException {
        ...
        for (Voeu v : voeux) {
            ...
            if ( candidatsAvecRepondeurAutomatique.contains(v.id.gCnCod) && v.estProposition()  &&!v.estAffecteHorsPP()) {
    ...
    public static void verifierP73(
        ...
        for (Voeu v : voeux) {
            ...
            if (v.estDemissionAutomatiqueParRepondeurAutomatique()) {
                ...
                if (proposition == null
                        || (!proposition.estAcceptationAutomatique() || proposition.getRangPreferencesCandidat() <= 0)
                        || (v.estDemissionAutomatiqueVoeuAttenteParRepondeurAutomatique() && proposition.getRangPreferencesCandidat() > v.getRangPreferencesCandidat())) {
    ...
    public static void verifierP75(Collection<Voeu> voeux,Set<Integer> candidatsAvecRepondeurAutomatique) throws VerificationException {
        ...
        for (Voeu v : voeux) {
            int gCnCod = v.id.gCnCod;
            if (candidatsAvecRepondeurAutomatique.contains(gCnCod) && v.getRangPreferencesCandidat() > 0) {
